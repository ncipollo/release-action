...
| generateReleaseNotes       | Indicates if release notes should be automatically generated. If `true`, these release notes will always be generated regardless of whether `omitBody` is `true` when creating a release and whether `omitBodyDuringUpdate` is `true` when updating a release. If this is `true` and `omitBodyDuringUpdate` is `true` when updating a release, the release body will be overwritten with only generated release notes. | false      | false                |
...
| omitBodyDuringUpdate       | Indicates if the release body should be omitted during updates. The body will still be applied for newly created releases. This will preserve the existing body during updates. If `generateReleaseNotes` is `true` and this is `false`, the body will be replaced with only release notes on an update.                                                                                                               | false      | false                |
